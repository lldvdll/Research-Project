# now.md archive — everything that predates the 800/900 series

Split out of `now.md` on 2026-09-11 so the dashboard could go back to being one screen.
**Nothing here is current.** It is the 100/200/300-series stipulations, findings, checklist and
decisions as they stood before the 800/900 build pass, kept verbatim as source material.

For current state read `now.md`; for the report pass read `report_progress.md`; for any
individual pre-800 number read `progress.md`, which re-derives them from the saved arrays.
