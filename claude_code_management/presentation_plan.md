...
> **Status (2026-08-10).** This plan is carried over **verbatim** from the 2026-08-10 session and is
> the authoritative record of what was decided. It has **not** been revised against the
> consolidation. Three `⚠ FLAG` notes were added below where a slide depends on a claim that
> `knowledge_base.md` §9.3 records as disputed or unsound. The flags are the only additions; no
> slide content was changed.
>
> Before rewriting this plan, adjudicate the open items in `knowledge_base.md` §9.3.
...
> ⚠ **FLAG — this slide's argument is currently unsound.** `knowledge_base.md` §4.1 derives
> output-layer suppression for **softmax cross-entropy**, but the implementation uses **squared
> error**, where suppression has a fixed point at zero (§4.4) rather than running away. The
> derivation must be redone for MSE before this slide asserts it. Tracked as §10.2 item 6. The
> "Work" bullet above already names this; it is restated here because the slide's contents depend
> on it, not just its preparation.
...
> ⚠ **FLAG — no prior number from this comparison is citable.** The earlier four-method run
> (experiment 11) was **2 tasks × 2 classes, not 5×2** — verified from the code, `knowledge_base.md`
> §9.3 D1 — and it saved no arrays, so its two conflicting retention figures (≈37% vs ≈68%, D2)
> cannot be reconciled. E4 is a fresh 5×2 run and is the **first** citable evidence for this slide.
> Do not carry any figure from §6.2 onto it, and make sure E4 writes its arrays to `.npz`.
...
> ⚠ **FLAG — do not let the retired framing back in.** The "interference claim has a scenario
> boundary" thesis was **retired as premature** on 2026-08-10 (`knowledge_base.md` §9.1, §10.1) and
> this slide is where it would most naturally reappear. State the structural difference and report
> what is observed; attribute only after E4, E5 and E6 are all in.
