...
def C(status, q, media, cap, note=None):
    ...
    p = f'<span class="pri p{pri}">P{pri}</span>' if pri else ""
    return (f'<figure class="card {cls}"><div class="chiprow"><div class="chip {cls}">{lab}</div>{p}</div>'
            f'<h4>{q}</h4>'
...
# ---------------------------------------------------------------- 6  (prospective configuration)
T.append(dict(n=6, q="Is the difference actually caused by prospective configuration?", v="gap",
  vt="The mechanism at the centre of the thesis is never measured against the outcome. Largest conceptual gap.",
  cols=[
    col(C("ok", "Does settling reach the weights, and which ones?",
      pic("344_weight_step_vs_lr.png"),
      "Realised weight step against learning rate, log–log. W1 shows <strong>no differential "
      "damping</strong> (backprop 0.93, PC 0.90 — both ≈linear). W2 does: <strong>0.99 against "
      "0.78</strong>. PC's output update multiplies the same error against the <em>settled</em> "
      "hidden activity, not the feedforward one — so prospective configuration reaches W2 by a "
      "route the hypothesis did not predict.",
      "the one existing measurement that looks inside the mechanism rather than at its outcome. It "
      "refuted its own pre-registered prediction, which is why it belongs in the argument rather "
      "than an appendix.")),
    col(C("crop", "How far does the hidden state actually move during settling?",
      pic("334_pc_settle_trace_by_dt_log.png"),
      "The displacement <strong>D = ‖x*(settled) − x(feedforward)‖</strong> is already computed on "
      "every update and logged — it is the quantity plotted here. Its plateau is non-zero by "
      "construction: that residual <em>is</em> prospective configuration.",
      "we currently read this only as a convergence diagnostic — does it flatten, and when. The "
      "same array is the mechanism variable, and has never been used as one.")),
    col(C("build", "Does how far the state moves predict how much is forgotten?", P(SK_DX, 1),
      ...
      "resulting ‖ΔW‖, both scenarios. This is the missing link in the chain — "
      "<strong>PC dynamics → internal configuration → weight change → forgetting</strong>. "
      "Without it the report shows PC behaves differently but never that its defining mechanism "
      "is why.",
      "<code>handle[\"diag\"][\"displacement\"]</code> is already published on every train step, so "
      "this is instrumentation of existing runs rather than a new experiment. An examiner can "
      "otherwise ask, fairly, where the evidence is that prospective configuration has anything "
      "to do with the result.")),
  ],
  why="Everything up to here is behavioural: the rules differ slightly, systematically, and by "
      "scenario. None of it touches the thing that makes PC <em>PC</em> — that the hidden state "
      "relaxes to a configuration the feedforward pass would not have produced, and the weight "
      "update is computed against that configuration instead. 344 is the only existing result "
      "inside that mechanism, and it is a good one precisely because it refuted its own "
      "prediction: the damping appears in the output layer, not the hidden layer, because "
      "settling reaches W2 through the presynaptic term. <strong>What is missing is the "
      "correlation with the outcome.</strong> The displacement is measured on every update and "
      "thrown away as a convergence check. Relating it to ΔW and to retention converts the "
      "report's spine from \"task structure → forgetting → PC vs backprop\" into \"task structure "
      "→ network dynamics → prospective configuration → weight change → forgetting\", which is "
      "the claim the title implies."))

# ---------------------------------------------------------------- 7
T.append(dict(n=7, q="Why are these two investigations, not one?", v="gap",
...
# ---------------------------------------------------------------- 8
T.append(dict(n=8, q="Class-IL — evidence for an output-competition component", v="gap",
  vt="Four results point one way, one points the other — and that contradiction narrows the candidates rather than just muddying them.",
  cols=[
    ...
    col(C("build", "Is it the task-1 weights being pushed down, or the competition at argmax?",
      P(SK_PFREEZE, 1),
      "Proposed: freeze <strong>only the task-1 columns of W2</strong> during task 2. Masking "
      "is train-time only — <code>active_vector</code> zeroes the error in "
      "<code>output_error</code>, while <code>predict</code> still argmaxes over all ten units. "
      "So masking spares the task-1 output weights <em>while letting task-2 units keep "
      "learning</em>; freezing all of W2 spares them but also blocks task-2's readout path, "
      "forcing task 2 into W1. A column-wise freeze separates those for the first time.",
      "the decisive experiment, and it is cheap. If it reproduces masking's +17.5, the mechanism "
      "is established as task-1 readout weights being driven down, and the freeze null was an "
      "artefact of crippling task-2 learning. If it recovers nothing, masking works through "
      "something other than sparing those weights. <strong>Needs approval — "
      "<code>_apply_freeze</code> is whole-matrix and would need a column mask.</strong>"),
  ...
  why="Four things point at output competition: digit identity, the output specification, "
      ...
      "easiest to get. The section title says <em>component</em>, not <em>problem</em>, because "
      "the evidence supports the former and not yet the latter. <strong>But the contradiction "
      "narrows the candidates rather than merely muddying them.</strong> Masking acts only on the "
      "training error, leaving argmax to compete over all ten units at evaluation, and it "
      "recovers +17.5. Freezing all of W2 also spares the task-1 output weights, and recovers "
      "nothing — but it additionally blocks task 2 from learning through the readout, forcing it "
      "into W1 and damaging the shared trunk. That single difference is testable by freezing only "
      "the task-1 columns, and it splits cleanly: if the gain returns, the mechanism is those "
      "weights being driven down; if it does not, masking works through something else entirely. "
      "The trained probe attacks the same ambiguity from the representation side and the "
      "per-layer path figure from the weight side. Until at least one lands, the localisation is "
      "<em>suggested</em>, not established."))

# ---------------------------------------------------------------- 9
T.append(dict(n=9, q="Domain-IL — evidence for a representation-dependent failure", v="gap",
...
# ---------------------------------------------------------------- 10
T.append(dict(n=10, q="What actually helps?", v="part",
  ...
  cols=[
    ...
    col(C("ok", "Does sparsity gating help, as the PC literature suggests?",
      ...
      "Domain-IL. An intervention motivated by the predictive-coding literature fails here, and "
      "fails PC hardest.",
      "the reading that PC's relaxation depends on the full hidden code is a "
      "<strong>hypothesis this result is consistent with, not something it establishes</strong>. "
      "Report the asymmetry; flag the explanation as untested."),
...
<p class="lede">Ten questions is the right granularity for deciding what to build and what to trust.
...
<ol>
<li><b>R1 · the phenotype</b><span class="sx">§1 + §4</span> — two qualitatively different
forms of forgetting. Changing the scenario changes the geometry; changing the rule does not.</li>
<li><b>R2 · PC is PC</b><span class="sx">§2 + §3</span> — a stable operating regime where the
...
<li><b>R3 · the comparison</b><span class="sx">§5 + §6</span> — PC ahead in Class-IL, behind in
Domain-IL, and the sign difference <em>amplifies with depth</em>. Then: does prospective
configuration itself account for it?</li>
<li><b>R4 · why two scenarios</b><span class="sx">§7</span> — repeated alternation. Class-IL
ping-pongs between incompatible solutions; Domain-IL contracts toward a joint one.</li>
<li><b>R5 · localisation</b><span class="sx">§8 + §9</span> — output competition on one side,
representation-dependent failure on the other, both stated as evidence rather than conclusion.</li>
<li><b>R6 · does fixing it help</b><span class="sx">§10</span> — the intervention ranking, and
