...
T.append(dict(n=1, q="What does forgetting look like?", v="build",
  vt="One figure, rebuilt. What exists today understates its own point.",
  cols=[
    col(C("build", "What happens to the task you just left?", P(SK_DEF, 2),
      ...
      "output-competition mechanism. Planting it here means §7 explains something the reader has "
      "already seen.")),
  ...
  why="This figure defines rather than compares, and a single fixed-budget seed does that better "
      "than a ten-seed mean: nothing is smoothed, the whole trajectory is visible, and the "
      "per-class lines carry a mechanism the reader needs later. Everything downstream refers to "
      "three things named here — the switch, the plateau, and the crossing."))

# ---------------------------------------------------------------- 2
T.append(dict(n=2, q="The setup, and why each choice was made", v="part",
  vt="Ordered generic → forgetting-specific. The metric is the climax of the section, not a footnote in it.",
  cols=[
    col(C("ok", "Can the trunk do enough work to ask mechanistic questions?",
      ...
      "thereafter. <strong>H=4 and H=8 are capacity-limited</strong> — flagged here, and it "
      "returns in §4 where PC reverses at H=4.", None)),
    col(C("build", "Does the conclusion depend on the activation?", P(SK_ACT, 2),
      ...
      "<strong>There is currently no justification for tanh at all</strong> — it was inherited, "
      "and Song &amp; Bogacz use sigmoid.",
      "a cheap three-point sweep, and the same insensitivity argument that works for the output "
      "maths below. A hole an examiner will find; an afternoon to close."),
      C("crop", "Does it depend on the output maths?",
    ...
    col(C("ok", "Does PC's settling reach a fixed point, and does dt move it?",
      ...
      "Domain-IL) — dt changes the route, not the destination. dt ≥ 0.7 lands somewhere else. "
      "That is the whole control: a stable band exists, and the choice inside it is free.", None),
      C("ok", "Does that band survive a change of architecture?",
      pic("347_pc_settle_dt_by_depth.png"),
      "The <strong>only</strong> unstable cell in the grid is dt = 0.4 at depth ≥ 2; dt = 0.2 "
      "converges everywhere. The band <em>shrinks with depth</em>, so this is a "
      "per-configuration precondition, not a one-time decision — ignoring that cost two sweep "
      "re-runs.",
      "settling was four figures and is now two. The cost curve (330) is tuning detail for an "
      "appendix; the displacement magnitude (346) is mechanism and moves to §5.")),
    col(C("build", "Why pair on seeds, and what does pairing buy?", P(SK_PAIR, 2),
      ...
      "numbers exist in 69, never drawn. §4's headline effects are one to three points, so this "
      "pre-empts the objection that they sit inside noise.")),
    col(C("amend", "Does an endpoint metric survive a change of stopping point?",
      pic("111_metric_sensitivity_to_threshold.png"),
      "<strong>Backprop alone — no rule comparison in this argument.</strong> Final task-1 falls "
      "monotonically to 0.28 as the threshold moves; S&amp;B mean error is U-shaped. An endpoint "
      ...
      "it</em>. That is the reason, and it needs no PC."),
      ...
      "backprop seeds at lr = 0.16</strong>. Two candidate metrics, two different failure regimes "
      "— which is what forces a competence-matched stopping rule rather than either metric alone.",
      "this pair <em>is</em> the metric decision, and it is the section's conclusion rather than "
      "its plumbing. The stopping rule is derived from two single-rule failures, not asserted."),
      feature=True),
  ],
  why="Ordered from choices any network needs to the choices this question needs: architecture, "
      "then specification, then PC's own control, then statistics, then the metric. The metric "
      "column is last because it is the most forgetting-specific decision in the report and the "
      "one everything downstream rests on — it is the section's conclusion, not a footnote. "
      "Critically, <strong>every justification here is single-rule</strong>. Choosing crossover "
      ...
      "already forbids for learning rates. Here the endpoint fails as the stopping point moves, "
      "crossover fails as the learning rate rises, and the competence-matched rule falls out of "
      "the two failures. The rule comparison is not mentioned until §3."))

# ---------------------------------------------------------------- 3
T.append(dict(n=3, q="Does each rule forget, and does it look the same in each scenario?", v="ok",
  ...
  cols=[
    col(C("ok", "Backprop, Class-IL — where does a run end up?",
      ...
      "this and the three beside it should be one 2×2 grid — scenario across, rule down. Assembly "
      "only, no retraining."),
    ...
    col(C("ok", "Backprop, Domain-IL — where does the same run end up?",
      ...
      "matches backprop's; the scenario, not the rule, sets the geometry.", None)),
  ],
  why="Read as a 2×2 these four panels make a point no single panel makes: <strong>changing the "
      ...
      "— one collapsing onto an axis, one stopping on a shelf. That asymmetry is what §6 has to "
      "justify formally, and it is why §4's comparison must be reported per scenario."))

# ---------------------------------------------------------------- 4
T.append(dict(n=4, q="Which rule is better, and is that stable?", v="part",
  ...
  cols=[
    ...
    col(C("ok", "Does more capacity rescue PC?", pic("341_width_sweep_accuracy.png"),
      ...
      "shrinks with width. At <strong>H=4 it reverses hard: −4.14 ± 0.70</strong>, surviving the "
      "settle correction.", None),
      ...
      "split in both directions</strong>: Class-IL +1.42 → +3.44, Domain-IL −0.72 → −3.49. "
      "Contradicts the pre-100 \"depth doesn't matter\" line.", None)),
    col(C("build", "Is the sign reversal stable across every axis we can vary?", P(SK_CONSOL, 1),
      ...
      "three existing scripts' arrays, one figure, no retraining. The section's headline, and it "
      "does not yet exist as a single object.")),
    col(C("amend", "How large is a real fix, on the same axes?", pic("340_lr_sweep_diff.png"),
      ...
      "retention</strong> against PC's +0 to +2. Flat across the whole grid, so not a tuning "
      "artefact. PC's effect is real and roughly a seventh the size.",
      "replay's magnitude crushes PC's line flat against the zero axis. Give replay its own scale "
      "— the point is the ratio, and one of the two terms is currently unreadable."),
      ...
      "reported <em>after</em> the result, as robustness. Using it to select the metric would be "
      "circular; using it to check one already fixed on other grounds is not.")),
  ],
  why="The answer is small, real, and sign-flipped by scenario. A one-to-three-point effect at a "
      ...
      "<em>systematic</em>, growing with depth in both directions at once. The replay panel keeps "
      "it honest: the problem is solvable, and PC's uplift is about a seventh of what a memory "
      "mechanism buys, so this is a small real effect rather than a headline."))

# ---------------------------------------------------------------- 5
T.append(dict(n=5, q="Is the difference actually caused by prospective configuration?", v="gap",
  vt="The mechanism at the centre of the thesis — and the one the source paper credits — is never measured against the outcome.",
  cols=[
    col(C("rerun", "Does PC align its updates toward the solution better than backprop?",
      P(SK_TA, 1),
      "<strong>Song &amp; Bogacz's own credited mechanism</strong>, and the report cannot omit it. "
      "Measured once at 5 seeds across four rules: alignment <em>does not track forgetting</em>, "
      "and ranks the rules opposite to retention. Two panels — alignment through training, and "
      "alignment against retention, which is flat.",
      "re-run as backprop vs PC at 10 seeds under the 300-series protocol. This was previously "
      "filed as an appendix negative; that was wrong. A direct test of the source paper's "
      "mechanism belongs in the argument whichever way it falls.")),
    col(C("ok", "Where does settling actually reach the weights?",
      pic("344_weight_step_vs_lr.png"),
      "Realised weight step against lr, log–log. W1 shows <strong>no differential damping</strong> "
      "(0.93 vs 0.90). W2 does: <strong>0.99 vs 0.78</strong>. PC's output update multiplies the "
      "same error against the <em>settled</em> hidden activity — so settling reaches W2 by a "
      "route the hypothesis did not predict.",
      "the one existing result that looks inside the mechanism. It refuted its own pre-registered "
      "prediction, which is why it belongs here rather than in an appendix."),
      C("crop", "How far does the state move, and does that depend on configuration?",
      pic("346_pc_settle_trace_by_depth_width_log.png"),
      "The displacement <strong>D = ‖x*(settled) − x(feedforward)‖</strong> is computed on every "
      "update. Its plateau is non-zero by construction — that residual <em>is</em> prospective "
      "configuration, and it varies with width and depth.",
      "moved here from the settling section. Read as mechanism, not as a convergence check.")),
    col(C("build", "Does how far the state moves predict how much is forgotten?", P(SK_DX, 1),
      "Proposed: per-seed settling displacement against task-1 retention, and against the "
      "resulting ‖ΔW‖, both scenarios. The missing link — <strong>PC dynamics → internal "
      "configuration → weight change → forgetting</strong>. Without it the report shows PC "
      "behaves differently but never that its defining mechanism is why.",
      "<code>handle[\"diag\"][\"displacement\"]</code> is already published on every train step, so "
      "this instruments existing runs rather than adding an experiment.")),
  ],
  why="Everything up to here is behavioural. None of it touches what makes PC <em>PC</em> — that "
      "the hidden state relaxes to a configuration the feedforward pass would not produce, and "
      "the update is computed against that. This section closes the gap from two directions. "
      "<strong>Target alignment is the mechanism the source paper credits</strong>, so testing it "
      "is not optional; that it fails is a result about their claim, not a tidy-up. The "
      "displacement is our own measure of the same thing and is thrown away on every update as a "
      "convergence check. Relating it to ΔW and retention converts the spine from \"task "
      "structure → forgetting → PC vs backprop\" into \"task structure → network dynamics → "
      "prospective configuration → weight change → forgetting\" — the claim the title implies."))

# ---------------------------------------------------------------- 6
T.append(dict(n=6, q="Why are these two investigations, not one?", v="gap",
  vt="The strongest section available, and almost none of it is built. Weight-space geometry is the part that makes it more than a pretty plot.",
  cols=[
    col(C("rerun", "Under repeated alternation, does the network converge on a joint solution?",
      ...
      "inward</strong> toward the joint corner for all three rules — exactly the predicted "
      "geometry.",
      "<strong>Domain-IL only.</strong> Pre-300, 5 seeds, includes replay. Re-run as backprop vs "
      "PC under current controls and add the Class-IL arm, which has never existed.")),
    col(C("build", "Do the two scenarios produce different geometry, not just different numbers?",
      P(SK_REPEAT, 1),
      "Proposed: both scenarios side by side. Prediction — Domain-IL spirals in (converging on a "
      "joint solution), <strong>Class-IL closes a loop</strong> (ping-ponging between "
      "incompatible solutions, learning nothing cumulative).",
      "a one-scenario addition to a script that already draws this. The separating quantity is "
      "already computed — the weight-state distance between successive same-task blocks, which "
      "vanishes for a spiral and does not for a loop. Shape becomes a <em>number</em>.")),
    col(C("build", "Are the weights themselves converging, orbiting, or drifting?", P(SK_PCA, 1),
      "Proposed: PCA of the weight trajectory under alternation, W1 and W2 separately. Accuracy "
      "can look settled while parameters wander, so <strong>this is the stronger version of the "
      "panel to its left</strong> — and it asks whether the trunk contracts while the readout "
      "orbits, which is §7's claim stated in weight space.",
      "68 already stores the full W1 trajectory (600 × 6272 = 196×32, seed 0), so the Domain-IL "
      "W1 panel is computable from saved arrays today. W2 and the Class-IL arm need the same "
      "re-run as the panel to the left — one experiment, both figures."),
      C("build", "Is forgetting about where you start in weight space?", P(SK_JOINT, 1),
      "Proposed: train on the <strong>joint</strong> distribution first, then run the sequential "
      "protocol from there, against sequential-from-scratch. If a network that already solves "
      "both tasks still collapses on task 1, forgetting is not a failure to <em>find</em> a joint "
      "solution — it is a failure to <em>stay</em> in one.",
      "new experiment, and the one that turns this section from description into a claim about "
      "parameter organisation rather than layer location. Pairs directly with the PCA panel: does "
      "joint initialisation turn the Class-IL loop into a spiral?")),
    col(C("build", "Which measurements separate by scenario, and which do not?", P(SK_TIE, 3),
      ...
      "placed last, as a tie-out into §7 and §8 rather than an opener. It summarises work the "
      "reader has just seen.")),
  ],
  why="The split must be earned twice, and it is earned decisively on one axis and not at all on "
      "another. <strong>Structurally it is not arguable</strong>: Class-IL has five output units "
      "receiving no positive target during task 2, so suppression is physically available; "
      "Domain-IL's shared units make it impossible. <strong>Empirically the geometry argument is "
      "the strongest available</strong> — a loop and a spiral are different kinds of behaviour, "
      "not different amounts of one — and the weight-space panels are what stop it being a pretty "
      ...
      "progressively contracts\". The joint-pretraining panel then asks the sharper question the "
      "others set up: whether the difference is about finding a joint solution or holding one. "
      "The tie-out draws the two measurements that behave the <em>same</em> in both scenarios as "
      "parallel lines rather than hiding them."))

# ---------------------------------------------------------------- 7
T.append(dict(n=7, q="Class-IL — evidence for an output-competition component", v="gap",
  vt="Four results point one way, one points the other — and that contradiction narrows the candidates rather than muddying them.",
  cols=[
    col(C("build", "It collapses to zero most of the time — when doesn't it, and why?",
      ...
      "overlaid. Known: two groups, near-zero and 15–30%. <strong>Averaging reports a number no "
      "individual run produces.</strong> Digit identity explains the split — 8, 9, 5 in task 1 "
      "raise retention, 6 lowers it.",
      "replaces the bar chart. The question is what moves the <em>distribution</em>, not the mean, "
      "and it merges the shape question with the why question."),
      ...
      "the evidence behind the distribution figure, not a substitute for it.")),
    col(C("amend", "Does protecting a layer during task 2 recover anything?",
      ...
      "+0.15 ± 0.24 Domain-IL. Freezing the hidden layer is strongly negative. The result that "
      "contradicts the readout story.",
      "fold into the distribution figure as an overlay. Freeze-both is absent because crossover "
      "is undefined on 0/10 seeds — a result, not a gap."),
      ...
      "unreadable, and the steps near 1700 are seeds leaving the mean, not behaviour. <strong>Also "
      "re-run:</strong> NCM is centroid-only with almost no dynamic range (69.9% at H=32 against "
      "an 81.2% raw-pixel baseline). Keep the task-2 reversal; replace the probe.")),
    col(C("build", "Is it the task-1 weights being pushed down, or competition at argmax?",
      P(SK_PFREEZE, 1),
      ...
      "<code>predict</code> still argmaxes over all ten units. So masking spares those weights "
      "<em>while letting task-2 units learn</em>; freezing all of W2 spares them but blocks task "
      "2's readout path, forcing it into W1.",
      "the decisive experiment, and cheap. If it reproduces masking's +17.5 the mechanism is "
      "established; if not, masking works through something else. <strong>Needs approval — "
      "<code>_apply_freeze</code> is whole-matrix.</strong>"),
      C("build", "Is the code intact, or is the probe too blunt to see the damage?",
      P(SK_PROBE, 1),
      "Proposed: a trained/refit linear probe against argmax, both tasks, across the run. Probe "
      ...
      "linearly decodable), masking (does removing competitors restore it). NCM answers none "
      "well.")),
    col(C("build", "Does the trunk settle while the readout keeps thrashing?", P(SK_PATH, 1),
      "Proposed: per-layer weight step per alternation block, from §6's runs. If <strong>W1 "
      "converges while W2 keeps oscillating</strong>, that is direct evidence the trunk found a "
      "joint solution and the readout is being dragged between two — the Class-IL claim, stated "
      "in weights rather than accuracy.",
      "shares §6's training runs and its PCA panel, so it costs one analysis, not one experiment. "
      "68 stores only a scalar weight gap per block; per-layer instrumentation needs adding.")),
  ],
  why="Four things point at output competition: digit identity, the output specification, "
      "masking's success, and the argmax-versus-probe gap. One points away — <strong>freezing the "
      "output layer recovers nothing</strong> — and it is the confirmation that should have been "
      "easiest to get. The title says <em>component</em>, not <em>problem</em>, because the "
      "evidence supports the former. <strong>But the contradiction narrows the candidates.</strong> "
      "Masking acts only on the training error, leaving argmax competing over all ten units at "
      "evaluation, and recovers +17.5. Freezing all of W2 also spares the task-1 output weights "
      "and recovers nothing — but additionally blocks task 2 from learning through the readout, "
      "forcing it into W1. That single difference is testable by freezing only the task-1 columns, "
      "and it splits cleanly. The trained probe attacks the same ambiguity from the representation "
      "side and the per-layer path from the weight side. Until one lands, the localisation is "
      "<em>suggested</em>, not established."))

# ---------------------------------------------------------------- 8
T.append(dict(n=8, q="Domain-IL — evidence for a representation-dependent failure", v="gap",
  ...
  cols=[
    col(C("ok", "Does which classes share an output unit predict what survives?",
      ...
      "Pooled r = +0.509 — real but far weaker, and absent from an independent block.", None)),
    col(C("crop", "Does the architecture of the trunk matter more here?",
      ...
      "representation-side finding — which is why it belongs here rather than as robustness in §4.",
      "take the Domain-IL column. Seed spread here exceeds any hyperparameter effect, so the "
      "variation is a property of the task draw, not the configuration.")),
    col(C("build", "Where does the damage live, when suppression is impossible?", P(SK_DRIFT, 1),
      ...
      "here that is expected, not informative.",
      "the largest genuine gap. §6's PCA and joint-pretraining panels are the other half of this: "
      "if Domain-IL's weights contract toward a joint solution, the damage is in how far that "
      "solution sits from task 1's, not in a layer.")),
  ],
  why="Class-IL has five output units receiving no positive target, so it is an output-competition "
      "problem. Domain-IL has none, so whatever happens is in the representation or the shared "
      "readout — a <strong>different investigation, not the same one with smaller effects</strong>. "
      "The depth asymmetry supports that directly: a trunk variable helps one scenario and hurts "
      "the other. What this section lacks is a positive answer to \"where\". The pairing "
      "correlation is a clear negative result — publishable as a correction, not an explanation; "
      "freezing rules out the output layer uninformatively; no drift measurement exists under the "
      "current protocol. Reporting that plainly beats importing §7's readout story by analogy, "
      "which the Domain-IL data does not support."))
...
T.append(dict(n=9, q="What actually helps?", v="part",
  ...
  cols=[
    col(C("build", "Of everything we can add, what recovers retention?", P(SK_INTERV, 2),
      ...
      "six experiments' arrays on one axis. Masking is Class-IL only — no absent classes to mask "
      "in Domain-IL, the same structural fact §6 rests on. Censoring annotated per bar.")),
    col(C("ok", "Does sparsity gating help, as the PC literature suggests?",
      ...
      "this is consistent with, not something it establishes</strong>. Report the asymmetry; flag "
      "the explanation as untested."),
      ...
      "1/10 and 3/10, and those that cross sit at the collapse floor.", None)),
    col(C("crop", "Does removing the competing units restore the readout?",
      ...
      "+7.59 ± 1.22 (mse)</strong>. The only intervention besides replay that moves Class-IL "
      "substantially — and it works by changing the readout, not the rule.",
      "take the masked-versus-unmasked half; the spec comparison is §2's. Censoring is structural: "
      "masking keeps task 1 above task 2, so often there is no crossing.")),
  ],
  why="This is where the argument lands, and it lands as a ranking rather than a single result. "
      "The two interventions that work substantially — replay and masking — either store the old "
      "data or change the readout. The ones that barely work add a penalty to the existing rule. "
      "The one that actively harms is the one the predictive-coding literature most often "
      "proposes. <strong>Nothing that modifies credit assignment alone recovers much.</strong> "
      "Read against §4, where PC's uplift is about a seventh of replay's, that is the project's "
      "conclusion on one axis."))
...
.col.feature .card{border-color:var(--ac);box-shadow:0 0 0 1px var(--ac),var(--sh)}
...
<div class="map">
<h3>This is an audit, not the Results structure</h3>
<p class="lede">Nine questions is the right granularity for deciding what to build and what to trust.
It is the wrong granularity for a dissertation, which would read as a lab notebook —
question, answer, question, answer. The sections below collapse into six Results chapters,
and the collapse is where the argument lives.</p>
<ol>
<li><b>R1 · the phenotype</b><span class="sx">§1 + §3</span> — two qualitatively different
forms of forgetting. Changing the scenario changes the geometry; changing the rule does not.</li>
<li><b>R2 · the setup</b><span class="sx">§2</span> — a stable operating regime where the
dynamics converge to the same fixed point, so later differences are not settling artefacts.
Keep it short; it is a precondition, not a chapter.</li>
<li><b>R3 · the comparison</b><span class="sx">§4 + §5</span> — PC ahead in Class-IL, behind in
Domain-IL, and the sign difference <em>amplifies with depth</em>. Then: does prospective
configuration itself account for it?</li>
<li><b>R4 · why two scenarios</b><span class="sx">§6</span> — repeated alternation. Class-IL
ping-pongs between incompatible solutions; Domain-IL contracts toward a joint one.</li>
<li><b>R5 · localisation</b><span class="sx">§7 + §8</span> — output competition on one side,
representation-dependent failure on the other, both stated as evidence rather than conclusion.</li>
<li><b>R6 · does fixing it help</b><span class="sx">§9</span> — the intervention ranking, and
the scale that makes PC's one to three points readable.</li>
</ol>
</div>
