...
from src.methods import make_backprop, make_replay, make_eqprop, make_eqprop_gated
...
methods = {
    ...
    "eqprop+gate":  make_eqprop_gated(in_dim=IN_DIM, lr=EQP_LR, beta=EQP_BETA, dt=EQP_DT, max_steps=EQP_MAX_STEPS,
                                      settle_patience=EQP_SETTLE_PAT, gate_frac=GATE_FRAC, seed=SEED, device=DEVICE),
...
plt.ylim(0, 105)
plt.title("Does update gating mitigate CF? Class-IL 5x2  (eqprop / eqprop+gate / replay)")
plt.legend()
plt.tight_layout()
