...
Report series 2, decade 300 (first of the mechanism-first narrative -- see
claude_code_management/narrative_plan.md, section 1). Backprop only: rule identity is not
introduced until the 310s (NCM). Every control parameter is read from experiments/config_300.yaml
-- nothing here relies on src/protocol.py's own Protocol defaults.
...
Two report figures, saved separately so they can be placed independently in the LaTeX report:
    300_forgetting_by_scenario_class_il.png
    300_forgetting_by_scenario_domain_il.png
