...
Report series 2, decade 300, second script. Pure re-analysis of 300's already-saved runs plus a
cheap, fixed similarity matrix -- no new training. Answers the question raised discussing 300's
Domain-IL variance: does digit-pair similarity explain it, the way pre-100 work found (~62% of
variance, [EMPIRICAL], not re-verified here -- only the correlation itself is being checked)?
...
task-1 accuracy from 300's saved arrays. Seed 9 (the class_il outlier discussed alongside this)
is marked distinctly for continuity with 302.
"""
import sys
...
SEEDS = 10
OUTLIER_SEED = 9                      # flagged discussing 300 -- the class_il 32.3% seed
MARKER_COLOR = {"class_il": "tab:blue", "domain_il": "tab:green"}

# ---------------------------------------------------------------- 300's saved arrays (required)
_300 = {}
for s in SCENARIOS:
    p = Path(__file__).parent / f"300_forgetting_by_scenario_{s}.npz"
    if not p.exists():
        raise FileNotFoundError(f"{p} not found -- run 300_forgetting_by_scenario.py first")
    _300[s] = np.load(p, allow_pickle=True)
...
splits = [proto.tasks(seed) for seed in range(SEEDS)]
...
domain_finals = _300["domain_il"]["finals"]
classil_finals = _300["class_il"]["finals"]
domain_crossovers = _300["domain_il"]["crossovers"]
classil_crossovers = _300["class_il"]["crossovers"]
