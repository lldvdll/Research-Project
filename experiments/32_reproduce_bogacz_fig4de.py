...
WHAT v1 GOT WRONG (v1 found no difference between the rules; here is why)
    ...
    training set partial_num: 600 examples PER CLASS -> 3000 per task. dataset_utils says
                 "the number of datapoints to extract from each class". v1 used all ~30,000;
                 v2 briefly read this as 600 in total, which gave a third of the training per
                 iteration. 3000/500 is also six whole batches, so every update uses the full
                 500 examples -- with 600 the last batch of each iteration held 100 examples
                 and, under the summed loss, a fifth of the gradient of the others.
...
PARTIAL_NUM   = 600          # partial_num: 600, PER CLASS -> 3000/task, 6 batches/iter
#   -> 6 weight updates per iteration, 24 per task block, 504 across the analysed 84.
#   v2 ran 2/8/168 and was still descending steeply at iteration 160 (exp 31 panel A).
