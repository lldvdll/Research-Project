"""Does PC's sequential forgetting differ in shape from backprop's (310), at PC's own settled
control configuration?

Report series 2, decade 332. Exact copy of 310 in every respect except METHOD ("backprop" ->
"pc") and the settle-dynamics parameters PC alone needs -- everything else, including the figure
design itself, is unchanged on purpose: the comparison is only clean if nothing else moved. Every
other control parameter is read from config_300.yaml, same as 310.

dt=0.4 (330: minimum settle-steps in both scenarios, flat on every accuracy/forgetting metric,
comfortable margin before the 0.5->0.6 instability edge -- resolved from an initial 0.2 through
0.3 to 0.4 as sampling around the trough got finer). Settling uses the SAME ADAPTIVE STOPPING as
330/331's production path (stop_delta=1e-4, stop_patience=3, steps=100 as a generous cap, not a
fixed count) -- 331 validated that truncating to a small FIXED step count costs nothing past
about steps=5-10, and adaptive stopping at dt=0.4 already lands in exactly that neighbourhood on
its own (330's settle-steps-vs-dt trough), so there is no reason to hand-pick a fixed number here
instead of just using the same adaptive criterion as everywhere else in this series.

Two report figures, saved separately, same naming convention as 310:
    332_pc_forgetting_by_scenario_class_il.png
    332_pc_forgetting_by_scenario_domain_il.png
"""
import sys
import time
...
SEED_START_DEFAULT = 10

# ---------------------------------------------------------------- settled control (330/331)
OPTIMAL_DT = 0.4
SETTLE_STEP_CAP = 100      # generous cap, not a fixed count -- see docstring
SETTLE_DELTA_TOL = 1e-4    # same adaptive-stop criterion as 330/331
SETTLE_PATIENCE = 3
...
SEED_START = CFG["training"]["seed_start"] if not SMOKE else SEED_START_DEFAULT
...
def run_one(proto, seed, data):
    ...
    train_step, predict = build(proto, METHOD, seed, dt=OPTIMAL_DT, steps=SETTLE_STEP_CAP,
...
for scenario in SCENARIOS:
    ...
    # Explicit axes geometry in inches -- identical to 310's, so the two figures are visually
    # comparable at a glance rather than differing by incidental layout choices.
    FIG_W, FIG_H = 7.0, 6.8
