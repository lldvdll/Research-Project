"""Across depths from the established 1 hidden layer up to 4 (one more than experiment 34's
reading of the Song & Bogacz reproduction config, 784-32-32-32-5 -- three hidden layers), does
PC's retention/crossover difference from backprop hold, shrink, or reverse -- with replay
alongside, swept at the same depths, as the "what remembering actually looks like" reference?

Report series 2, decade 342. Last of the three-part BP-vs-PC comparison (narrative_plan.md's
S2/340 series), after 340 (lr) and 341 (width). Like 341 and unlike 340, replay is NOT held at a
fixed reference here -- depth is a shared architecture parameter, not a rule-specific tuning
knob, so all three methods (backprop, replay, pc) are trained at every depth point.

DEPTHS = [1, 2, 3, 4] hidden layers, all width 32 (the established H). src/model.py's Arch and
src/protocol.py's Protocol already support this directly (`n_layers`, `hidden` as int or tuple) --
no src/ change needed, this was built for exactly this during the (failed, archived) Song &
Bogacz reproduction and never removed (CLAUDE.md's code-conventions section).

Width is held at 32 (established). lr is held at each rule's own established default from
config_300.yaml (backprop 0.01, pc 0.02); replay is not in that dict, so it keeps its own
METHOD_DEFAULTS lr (0.05) automatically -- 340 is the check for whether that pairing is actually
each rule's best operating point; this script holds it fixed and varies depth only.

PC's settle control (dt=0.4, adaptive stop: stop_delta=1e-4, stop_patience=3, steps=100 cap) is
held fixed at its H=32/depth=1-validated value (330-334) -- NOT re-verified per depth here. The
literature flags target alignment degrading with depth for PC specifically ([R1] Fig 3b-e,
knowledge_base.md 12.1), which is a reason a deeper net might need different settle dynamics, not
a reason to assume this cell is safe -- `reached` makes a convergence failure visible either way,
even though it wouldn't distinguish "capacity" from "settle dynamics" as the cause.

FULL max_iters_per_task budget (config_300.yaml's 5000, not trimmed) -- same reasoning as 340/341.

SEEDS = 5 (10-14). --add-seeds=LO-HI merges 15-19 in later without rerunning existing cells.
--add-depth=X merges one new depth in (330's dt-merge pattern, generalised, as in 341's --add-width).
...
paired_wilcoxon; control=backprop) at every depth, on retention and crossover height. Replay's
own retention/crossover is printed as a benchmark, not paired-tested.

The depth=1 cell is trained fresh (not reused from 310/332/340/341) -- agreement across
independently-coded scripts on the same cell is a cheap correctness check, not redundant compute.

Two figures: 342_depth_sweep_accuracy.png (retention, crossover height) and
342_depth_sweep_speed.png (task-2 steps to 90%, crossover step), each 2 scenario columns x 2
metric rows, box-and-whisker per depth, one box per method (backprop, replay, pc).
...
_add_depth_arg = next((a for a in sys.argv if a.startswith("--add-depth=")), None)
ADD_DEPTH = int(_add_depth_arg.split("=")[1]) if _add_depth_arg else None
...
OPTIMAL_DT = 0.2   # not 0.4 -- see 345/346/347: dt=0.4 oscillates (not slow, genuinely wrong
                   # fixed point, confirmed by comparing its oscillation mean against dt=0.3's
                   # real converged value) at depth>=2. dt=0.2 is stable at every depth 1-4 AND
                   # at Class-IL's H=4 (341's own edge case), so it's used uniformly for both.
SETTLE_STEP_CAP = 100
...
BASE_PROTOCOL = replace(
    ...
    hidden=CFG["architecture"]["hidden"],   # width held at 32 -- see docstring
...
def _rows_for_seeds(proto, data, method, depth, seeds):
    return [(method, depth, seed) + _row_for(proto, method, seed, data) for seed in seeds]
...
if REPLOT and all(Path(array_path(s)).exists() for s in SCENARIOS):
    ...
    for scenario in SCENARIOS:
        ...
        for depth in DEPTHS:
            ...
            for method in METHODS:
                new_rows.extend(_rows_for_seeds(p, data, method, depth, ADD_SEEDS))
...
elif ADD_DEPTH is not None:
    if not all(Path(array_path(s)).exists() for s in SCENARIOS):
        raise RuntimeError("--add-depth needs existing saved data to merge into -- run the full "
                          "sweep first")
    t0 = time.perf_counter()
    train_rows = {}
    for scenario in SCENARIOS:
        loaded = list(np.load(array_path(scenario), allow_pickle=True)["data"])
        existing = [r for r in loaded if r[1] != ADD_DEPTH]
        seeds_present = sorted(set(r[2] for r in loaded)) if loaded else _seed_range()
        proto = replace(BASE_PROTOCOL, scenario=scenario, n_layers=ADD_DEPTH)
        data = load(proto)
        new_rows = []
        for method in METHODS:
            new_rows.extend(_rows_for_seeds(proto, data, method, ADD_DEPTH, seeds_present))
        train_rows[scenario] = existing + new_rows
        print(f"  {scenario:10s} added depth={ADD_DEPTH}   [{time.perf_counter() - t0:5.0f}s]")
    for scenario in SCENARIOS:
        np.savez(array_path(scenario), data=np.array(train_rows[scenario], dtype=object))
    print("saved arrays (merged)")

elif EXTEND_CAP is not None:
    # Re-runs ONLY the (method, depth, seed) cells that never reached threshold on task 1 or
    ...
    for scenario in SCENARIOS:
        ...
        redone = []
        for depth in sorted({r[1] for r in capped}):
            p = replace(proto, n_layers=depth)
            data = load(p)
            for method in sorted({r[0] for r in capped if r[1] == depth}):
                seeds = sorted({r[2] for r in capped if r[1] == depth and r[0] == method})
                redone.extend(_rows_for_seeds(p, data, method, depth, seeds))
    ...
    for scenario in SCENARIOS:
        ...
        for depth in DEPTHS:
            ...
            data = load(p)
            for method in METHODS:
                train_rows[scenario].extend(_rows_for_seeds(p, data, method, depth, _seed_range()))
...
# (method, depth, seed, retention, t2_final, cx_height, cx_step_rel, t1_steps, t2_steps,
...
def _vals(rows, method, depth, col):
    ...
    d = {int(r[2]): r[col] for r in rows if r[0] == method and r[1] == depth}
    return np.array([d[s] for s in sorted(d)], dtype=float)


def _cap_pct(rows, method, depth, reached_col):
    v = [r[reached_col] for r in rows if r[0] == method and r[1] == depth]
    return 100.0 * (1.0 - np.mean(v)) if v else float("nan")


# ---------------------------------------------------------------- console report
DEPTHS_SEEN = DEPTHS if not (REPLOT or ADD_SEEDS is not None or ADD_DEPTH is not None) else \
    sorted(set(r[1] for r in train_rows[SCENARIOS[0]]))

print("\npaired comparison (pc vs backprop control), by depth -- replay shown as a benchmark:")
for scenario in SCENARIOS:
    rows = train_rows[scenario]
    print(f"\n  {scenario}")
    for depth in DEPTHS_SEEN:
        bp_ret, pc_ret = _vals(rows, "backprop", depth, COL["retention"]), \
                         _vals(rows, "pc", depth, COL["retention"])
        bp_cx, pc_cx = _vals(rows, "backprop", depth, COL["cx_height"]), \
                      _vals(rows, "pc", depth, COL["cx_height"])
        rp_ret = _vals(rows, "replay", depth, COL["retention"])
        rp_cx = _vals(rows, "replay", depth, COL["cx_height"])
        ...
        cap_bp = _cap_pct(rows, "backprop", depth, COL["reached_t2"])
        cap_pc = _cap_pct(rows, "pc", depth, COL["reached_t2"])
        print(f"    D={depth:<4}  retention pc-bp {d_ret:+6.1f} +-{se_ret:4.1f}"
...
def _box_panel(ax, rows, col, ylabel, title):
    positions = np.arange(len(DEPTHS_SEEN))
    for method in METHODS:
        data_by_depth = [_vals(rows, method, d, col) for d in DEPTHS_SEEN]
        data_by_depth = [v[np.isfinite(v)] for v in data_by_depth]
        ax.boxplot(data_by_depth, positions=positions + METHOD_OFFSET[method], widths=0.18,
        ...
        medians = [float(np.median(v)) if v.size else np.nan for v in data_by_depth]
        ax.plot(positions + METHOD_OFFSET[method], medians, color=METHOD_COLOR[method],
               lw=1.4, alpha=0.9, zorder=3)
    ax.set_xticks(positions)
    ax.set_xticklabels([str(d) for d in DEPTHS_SEEN])
...
for col_i, scenario in enumerate(SCENARIOS):
    ...
    _box_panel(axes[0][col_i], rows, COL["retention"], "task-1 retention (%)",
              f"{scenario.replace('_', '-')}: retention vs depth")
    _box_panel(axes[1][col_i], rows, COL["cx_height"], "crossover height (%)",
              f"{scenario.replace('_', '-')}: crossover vs depth")
...
for col_i, scenario in enumerate(SCENARIOS):
    ...
    _box_panel(axes[0][col_i], rows, COL["t2_steps"], "steps for task 2 to reach 90%",
              f"{scenario.replace('_', '-')}: task-2 training length vs depth")
    _box_panel(axes[1][col_i], rows, COL["cx_step_rel"], "steps into task 2 at crossover",
              f"{scenario.replace('_', '-')}: crossover step vs depth")
