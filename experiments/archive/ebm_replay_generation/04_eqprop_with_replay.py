...
from src.methods import make_eqprop, make_eqprop_replay, make_replay
...
methods = {
    ...
    "eqprop+replay":  make_eqprop_replay(train, cidx, in_dim=IN_DIM, lr=EQP_LR, beta=EQP_BETA, dt=EQP_DT, max_steps=EQP_MAX_STEPS, settle_patience=EQP_SETTLE_PAT, per_class=RP_PER_CLASS, seed=SEED, device=DEVICE),
    "replay (bp)":    make_replay(train, cidx, in_dim=IN_DIM, lr=RP_LR, per_class=RP_PER_CLASS, seed=SEED, device=DEVICE),
}
...
plt.ylim(0, 105)
plt.title("Does replay help eqprop? Class-IL 5x2  (eqprop / eqprop+replay / replay (bp))")
plt.legend()
plt.tight_layout()
