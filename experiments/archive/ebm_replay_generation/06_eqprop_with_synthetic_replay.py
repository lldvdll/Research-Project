...
from src.methods import make_eqprop, make_eqprop_synthetic, make_eqprop_replay
...
methods = {
    "eqprop":            make_eqprop(in_dim=IN_DIM, lr=EQP_LR, beta=EQP_BETA, dt=EQP_DT, max_steps=EQP_MAX_STEPS, settle_patience=EQP_SETTLE_PAT, seed=SEED, device=DEVICE),
    "eqprop+synthetic":  make_eqprop_synthetic(in_dim=IN_DIM, lr=EQP_LR, beta=EQP_BETA, dt=EQP_DT, max_steps=EQP_MAX_STEPS, settle_patience=EQP_SETTLE_PAT, n_synth=RP_PER_CLASS, seed=SEED, device=DEVICE),
    "eqprop+real":       make_eqprop_replay(train, cidx, in_dim=IN_DIM, lr=EQP_LR, beta=EQP_BETA, dt=EQP_DT, max_steps=EQP_MAX_STEPS, settle_patience=EQP_SETTLE_PAT, per_class=RP_PER_CLASS, seed=SEED, device=DEVICE),
}
...
plt.ylim(0, 105)
plt.title("Do generated replay samples work? Class-IL 10x1  (backprop / replay / eqprop)")
plt.legend()
plt.tight_layout()
