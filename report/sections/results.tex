...
% --------------------------------------------------------------------- R1
\subsection{Two scenarios, two kinds of forgetting}
\label{sec:r1}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \panel{0.49\panelspan}{911_two_forgetting_phenotypes_a.png}\hfill
    \panel{0.49\panelspan}{911_two_forgetting_phenotypes_b.png}\par\vspace{6pt}
    \panel{0.49\panelspan}{911_two_forgetting_phenotypes_c.png}\hfill
    \panel{0.49\panelspan}{911_two_forgetting_phenotypes_d.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \solo{912_pc_minus_backprop_sweeps.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \solo{913_replay_sets_the_scale.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \panel{0.98\panelspan}{916_displacement_to_retention_a.png}\par\vspace{6pt}
    \panel{0.98\panelspan}{916_displacement_to_retention_b.png}
  \end{panelbox}
  \caption{\label{fig:916}\gap{916 not written; 803 has run}\wordcount{110}}
\end{figure*}

% --------------------------------------------------------------------- R4
\subsection{Why these are two investigations, not one}
\label{sec:r4}
% ⚠ HEADING CORRECTED. An earlier version titled this "What repeated alternation reveals about
% the trajectory", which mistook the evidence for the question. R4's job is to JUSTIFY THE
% SCENARIO SPLIT; repeated alternation, joint pre-training and the tie-out are the evidence that
% does it. Write the section as an argument for the split, not as a tour of three experiments.
...
%
% The scenario-separation numbers this section exists to deliver: PC - backprop is -0.72 +- 0.19
% after ONE switch and -16.71 +- 2.45 (6.8 sem) after FIVE task-2 blocks, in Domain-IL; Class-IL
% is -1.78 +- 3.09, i.e. nothing. A two-task protocol understates the scenario difference by more
% than twenty times, which is an argument about the benchmark as well as the rule.
%
...
% ⚠ 919 has a confound and Fig.~\ref{fig:919} draws it: both arms stop task 1 at the same
% competence, but the joint arm starts above threshold so its task-1 phase is far shorter
% (402 -> 182 updates). Correlation between phase-length difference and retention difference is
% -0.64 in Class-IL (confound present) and -0.09 for Domain-IL backprop (clean). LEAN ON THE
% DOMAIN-IL BACKPROP ARM. The two cannot be separated within this design.

\begin{figure*}[t]\centering
  \begin{panelbox}
    \solo{917_alternation_geometry.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \solo{919_joint_then_sequential.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \solo{920_what_separates_by_scenario.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \panel{0.98\panelspan}{927_overtrain_metric_grid.png}\par\vspace{6pt}
    \panel{0.98\panelspan}{928_forgetting_shape.png}
  \end{panelbox}
  \caption{\label{fig:927}\wordcount{130}}
\end{figure*}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \panel{0.98\panelspan}{921_retention_distribution_a.png}\par\vspace{6pt}
    \panel{0.98\panelspan}{921_retention_distribution_b.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \solo{922_partial_column_freeze.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \panel{0.98\panelspan}{923_probe_vs_argmax_a.png}\par\vspace{6pt}
    \panel{0.98\panelspan}{923_probe_vs_argmax_b.png}
  \end{panelbox}
...
\begin{figure*}[t]\centering
  \begin{panelbox}
    \panel{0.98\panelspan}{924_per_layer_weight_path.png}\par\vspace{6pt}
    \panel{0.98\panelspan}{924b_hidden_code_drift.png}
  \end{panelbox}
